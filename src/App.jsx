...
function QAToolsPanel({ currentUid, contracts, contractsLoading, onClose, onMigrate, migrationStatus }) {
  ...
  const handleMigrate = async () => {
    ...
    setIsLoading(true);
    await onMigrate(sourceUid.trim());
    setIsLoading(false);
  };
...
export default function Laosfactos() {
  ...
  const handleMigration = async (sourceUid) => {
    if (!user) return;
    try {
      ...
      setMigrationStatus({ success: true, ...result.data });
    } catch (e) {
      console.error("Migration error:", e);
      setMigrationStatus({ success: false, error: e.message });
    }
  };
